...
| Token | Value | Tailwind Class | Why it is present |
|-------|-------|----------------|-------------------|
| `input-sm` | `h-8 text-sm` | `h-8 text-sm` | Compact inputs. |
| `input-md` | `h-10 text-base` | `h-10 text-base` | **Standard input**. |
| `input-lg` | `h-12 text-lg` | `h-12 text-lg` | Large inputs. |
| `focus-ring` | Brand Color | `focus:ring-2 focus:ring-brand-500` | Accessibility focus state. |
| `error-ring` | Red | `ring-2 ring-red-500` | Validation error state.

---

## 15. Min Height (`minHeight`)

| Token | Meaning | Tailwind Class | Why it is present |
|-------|---------|----------------|-------------------|
| `0` | 0px | `min-h-0` | Reset min-height. |
| `full` | 100% | `min-h-full` | Fill parent height. |
| `screen` | Viewport | `min-h-screen` | **Full viewport height**. Essential for Hero sections. |
| `fit` | Fit Content | `min-h-fit` | Shrink to fit content. |
| `min` | Min Content | `min-h-min` | Minimum intrinsic size. |
| `max` | Max Content | `min-h-max` | Maximum intrinsic size. |

---

## 16. Animations (Entrance)

These tokens control how elements appear when scrolling into view. They are applied via the `animation` property in the node styles.

| Token | Effect | Description |
|-------|--------|-------------|
| `fade-in` | Opacity 0 -> 1 | Smooth fade in. Default fallback. |
| `slide-up` | Y +30px -> 0 | Slides up while fading in. Good for cards. |
| `slide-down` | Y -30px -> 0 | Slides down while fading in. Good for navbars. |
| `scale-up` | Scale 0.95 -> 1 | Subtle zoom in. Good for hero images. |
| `scale-down` | Scale 1.05 -> 1 | Subtle zoom out. Good for backgrounds. |
| `none` | No Animation | Static display. |

**Behavior:**
-   **Trigger**: `whileInView` (plays when element enters viewport).
