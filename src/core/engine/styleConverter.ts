...
export interface NodeStyles {
  ...
  borderBottomWidth?: keyof typeof tokens.border.width;
  ...
  // Grid
  columns?: keyof typeof tokens.grid.columns;
...
function convertSingleStyle(styles: Partial<NodeStyles>, prefix: string = ""): string[] {
  ...
  if (styles.typography && tokens.typography[styles.typography]) {
    const typographyClasses = tokens.typography[styles.typography]
      .split(' ')
      .map(c => `${p}${c}`);
  ...
  // Spacing
  if (styles.padding && paddingMap[styles.padding]) {
    classes.push(`${p}${paddingMap[styles.padding]}`);
  }
  if (styles.margin && marginMap[styles.margin]) {
    classes.push(`${p}${marginMap[styles.margin]}`);
  }
  if (styles.gap && gapMap[styles.gap]) {
    classes.push(`${p}${gapMap[styles.gap]}`);
  }
  
  // Section & Container Padding
  if (styles.sectionPadding && tokens.sectionPadding[styles.sectionPadding]) {
    const sectionClasses = tokens.sectionPadding[styles.sectionPadding]
      .split(' ')
      .map(c => `${p}${c}`);
  ...
  // Flex
  if (styles.flex && flexMap[styles.flex]) {
    classes.push(`${p}${flexMap[styles.flex]}`);
  }
  if (styles.flexBasis && flexBasisMap[styles.flexBasis]) {
    classes.push(`${p}${flexBasisMap[styles.flexBasis]}`);
  }
  if (styles.flexGrow) classes.push(`${p}grow`);
  if (styles.flexShrink) classes.push(`${p}shrink`);
  
  // ✅ CRITICAL FIX: Support both 'direction' and 'flexDirection'
  const direction = styles.direction || styles.flexDirection;
  if (direction && flexDirectionMap[direction]) {
    classes.push(`${p}${flexDirectionMap[direction]}`);
  }
  
  if (styles.alignItems && alignItemsMap[styles.alignItems]) {
    classes.push(`${p}${alignItemsMap[styles.alignItems]}`);
  }
  if (styles.justifyContent && justifyContentMap[styles.justifyContent]) {
    classes.push(`${p}${justifyContentMap[styles.justifyContent]}`);
  }
  ...
  // Visuals
  if (styles.radius && tokens.radius[styles.radius]) {
    classes.push(`${p}${tokens.radius[styles.radius]}`);
  }
  if (styles.shadow && tokens.shadow[styles.shadow]) {
    classes.push(`${p}${tokens.shadow[styles.shadow]}`);
  }
  if (styles.opacity && tokens.opacity[styles.opacity]) {
    classes.push(`${p}${tokens.opacity[styles.opacity]}`);
  }
  if (styles.zIndex && tokens.zIndex[styles.zIndex]) {
    classes.push(`${p}${tokens.zIndex[styles.zIndex]}`);
  }
  
  // Border
  if (styles.borderWidth && tokens.border.width[styles.borderWidth]) {
    classes.push(`${p}${tokens.border.width[styles.borderWidth]}`);
  }
  if (styles.borderBottomWidth && borderWidthMap[styles.borderBottomWidth]) {
    classes.push(`${p}${borderWidthMap[styles.borderBottomWidth]}`);
  }
  if (styles.borderColor && tokens.border.color[styles.borderColor]) {
    classes.push(`${p}${tokens.border.color[styles.borderColor]}`);
  }
  if (styles.borderStyle && tokens.border.style[styles.borderStyle]) {
    classes.push(`${p}${tokens.border.style[styles.borderStyle]}`);
  }

  // Text Align
  if (styles.textAlign && textAlignMap[styles.textAlign]) {
    classes.push(`${p}${textAlignMap[styles.textAlign]}`);
  }

  // Typography Modifiers
  if (styles.lineHeight && tokens.lineHeight[styles.lineHeight]) {
    classes.push(`${p}${tokens.lineHeight[styles.lineHeight]}`);
  }
  if (styles.fontWeight && tokens.fontWeight[styles.fontWeight]) {
    classes.push(`${p}${tokens.fontWeight[styles.fontWeight]}`);
  }
  if (styles.fontFamily && tokens.fontFamily[styles.fontFamily]) {
    classes.push(`${p}${tokens.fontFamily[styles.fontFamily]}`);
  }

  // Misc
  if (styles.aspectRatio && tokens.aspectRatio[styles.aspectRatio]) {
    classes.push(`${p}${tokens.aspectRatio[styles.aspectRatio]}`);
  }
  if (styles.transition && tokens.transition[styles.transition]) {
    classes.push(`${p}${tokens.transition[styles.transition]}`);
  }
  if (styles.easing && tokens.easing[styles.easing]) {
    classes.push(`${p}${tokens.easing[styles.easing]}`);
  }
